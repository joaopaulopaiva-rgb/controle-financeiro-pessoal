...
- **Ampliação da regra acima (14/set/2026):** além de guardar na coleção `unifan`, **também enviar a informação (já separada e categorizada) pra outra sessão do Claude Code chamada "Sistema de controle financeiro"** (`session_014bB3e7dxELY77Dz1a5W5Bt`), onde a pessoa e o Claude fazem o acompanhamento financeiro da Unifan de verdade (artifacts "Painel UNIFAN", "Margens de Escolas", "Alunos, Responsáveis e Valores"). Usar a ferramenta `SendMessage` com `to` = o nome/ID dessa sessão, mandando os dados do gasto (comerciante, valor, data, e categoria se já tiver ficado óbvia) pra que a sessão de lá decida como registrar no próprio sistema dela — essa sessão desta conversa **não** tenta escrever direto no banco/artifacts de lá, só repassa a informação. Depois de mandar, avisar a pessoa aqui o que foi enviado.
...
**Regra geral daqui pra frente** (ainda vale mesmo com abril resolvido, caso apareça outro mês parecido): o código em `app/financas.html` (`renderMesesMaiores`/`renderEvolucoes`/`agregarPeriodo`) já trata **`resumos/{mes}` como fonte pro histórico daquele mês sempre que existir**, mesmo que também haja `transacoes` soltos no mesmo mês — não soma os dois.

**Bug real corrigido em 04/set/2026 (duas partes, mesma conversa):**
- A pessoa reportou "os lançamentos de janeiro em diante não aparecem" ao testar no celular. Causa 1: o botão de cada mês no filtro (`renderMesesGrid`) só ficava clicável se o mês tivesse `transacoes` reais — um mês só-com-resumo (jan-abr) aparecia **travado/inclicável**, então a pessoa nunca conseguia selecioná-lo, nem abrir a "Análise do extrato" daquele mês (que já existia em `resumos`). Corrigido com `mesesDoAnoHistorico` (real **ou** resumo), usada em `renderMesesGrid` e no clique do ano em `renderAnos`.
- Causa 2 (reportada em seguida — "não tem valor associado a esses meses nas categorias"): mesmo depois de conseguir selecionar jan-abr, o **gauge** (taxa de poupança) e o **treemap** ("Categoria × subcategoria") continuavam vazios — eles só olhavam `transacoes`, e resumo não tem lançamento individual. Mas resumo **tem** total por categoria (`por_categoria`) e totais gerais (`despesas_total`/`receitas_total`), que são exatamente o que esses dois widgets precisam (nenhum dos dois usa subcategoria). Criada `agregarPeriodo()`: para cada mês selecionado, usa o resumo (se existir) — cada categoria vira um único bloco "Sem detalhe (resumo importado)" no treemap, sem quebra por subcategoria — senão soma as `transacoes` reais daquele mês como antes. `renderGauge`/`renderTreemap` passaram a usar essa função. **Top 5** (que é ranking por subcategoria) continua só em cima de `transacoes` — não tem como preencher sem subcategoria — mas a mensagem de vazio agora diferencia "sem despesa nenhuma" de "tem despesa, só não tem detalhe por subcategoria" (chamando atenção pro treemap logo abaixo). **Extrato completo** continua só em cima de `transacoes`, sem alternativa — não tem lançamento individual pra listar num mês só-resumo, isso é esperado.
- Terceiro pedido, mesma conversa: **o filtro passou a abrir sempre só com o mês atual selecionado** (`garantirPeriodo`). Antes, o padrão ao abrir o app era selecionar todos os meses com `transacoes` real no ano mais recente com dado (podia ser vários meses somados de uma vez). Pedido explícito da pessoa pra simplificar: abrir sempre enxuto, só o mês corrente, e escolher outros meses manualmente no filtro quando quiser.
